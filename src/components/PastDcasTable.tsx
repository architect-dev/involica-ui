...
const PastDcaRow: React.FC<{ dca: DCAStats; openModal: (hash: string) => void; censorable: boolean }> = ({ dca, openModal, censorable }) => {
...
export const PastDcasTable: React.FC<{ includeStatsLink?: boolean; censorable?: boolean }> = ({ includeStatsLink = false, censorable = false }) => {
	...
	return (
		<Card title='Executed DCAs' padding='24px'>
			...
			<SummitPopUp
				...
				popUpContent={
					modalDcaHash == null ? null : <DCATransactionModal dca={dcasData?.find(({ txHash }) => txHash === modalDcaHash)} onDismiss={hideTxModal} />
				}
			/>
		</Card>
	)

...
import { Nullable, bnDisplay, tryGetAddress } from '@utils'
...
const TokenRowButton = styled(RowBetween)<{ disabled: boolean }>`
	width: 100%;
	padding: 0 18px;
	height: 48px;
	min-height: 48px;
...
const TokenSelectRow: React.FC<{
	...
	return (
		<TokenRowButton disabled={disabledReason != null} onClick={() => (disabledReason != null ? null : setToken(token.address))}>
			<Row gap='6px'>
			...
			if (variant === 'tokenIn')
				return filteredTokens.sort(
					({ address: a, decimals: decA }, { address: b, decimals: decB }) =>
						Number(bnDisplay(userTokensData?.[b]?.balance, decB, 3)) - Number(bnDisplay(userTokensData?.[a]?.balance, decA, 3))
				)
			if (variant === 'tokenOut') return filteredTokens.sort(({ address: a }, { address: b }) => (getIsStable(a) ? 1 : -1) - (getIsStable(b) ? 1 : -1))

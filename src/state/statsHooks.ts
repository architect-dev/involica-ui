...
export const useInvolicaSnapshotsData = () => {
	...
	return useMemo((): InvolicaDCA[] | null => {
		...
		return data.involicaSnapshots.map((snapshot: any) => transformInvolicaSnapshot(snapshot))
	}, [loading, data])
}
export const useInvolicaUserDcasData = () => {
	...
	return useMemo((): InvolicaDCA[] | null => {
		...
		return data.dcas.map((dca: any) => transformInvolicaDCA(dca))
	}, [loading, data])
}
...
export const useInvolicaDcasOrSnapshots = (dcas: boolean) => {
	...
	return useMemo((): TimestampOutsTokensAmountsPrices[] | null => {
...
const incrementPriceUsdDisplay = (amount: number | null, price: number | null, current: PriceUsdDisplay, censored: boolean): PriceUsdDisplay => {
	if (current == null || amount == null) return getPriceUsdDisplay(amount, price, censored)
...
const getValueChangeAndStatusFromUsds = (currentUsd: number | null, tradeUsd: number | null, censored: boolean): ValueChangeAndStatus => {
	return {
		...getUsdDisplay((currentUsd ?? 0) - (tradeUsd ?? 0), censored),
...
export const useUserDcasData = (censored = false) => {
	...
	return useMemo((): DCAStats[] | null => {
		...
		return orderBy(dcas, ['timestamp'], ['desc']).map((dca): DCAStats => {
			const outTokens = dca.outTokens.map((outToken, outIndex) =>
				getFreshOrUpdatedTokenTradeData(tokensData[ethers.utils.getAddress(outToken)], dca.outAmounts[outIndex], dca.outPrices[outIndex], null, censored)
			)
			return {
				txHash: dca.txHash,
				...
				inToken: getFreshOrUpdatedTokenTradeData(tokensData[ethers.utils.getAddress(dca.inToken)], dca.inAmount, dca.inPrice, null, censored),
				outTokens,
...
export const useDailyTokenPrices = () => {
	...
	const [inTokens, outTokens] = useMemo(() => {
		...
		return [Object.keys(ins).map((address) => address.toLowerCase()), Object.keys(outs).map((address) => address.toLowerCase())]
	}, [txs])
		...
		while (dayTimestamp <= Math.floor(Date.now() / 1000) + 86400) {
			...
			for (let i = dcaIndex; i < dcasOrSnapshots.length; i++) {
				...
				dcaOrSnapshot.outTokens.forEach((outToken, outTokenIndex) => {
					...
					const price = dcas ? dcasOrSnapshots[i].outPrices[outTokenIndex] : parseFloat(dailyPrices[yesterdayTimestamp]?.[outToken] ?? '0')

...
export const ApproveIn: React.FC = () => {
	...
	const allowance = useMemo(() => {
		...
		return bnDisplay(tokenInUserData?.allowance, tokenInData?.decimals, 3)
	...
	const approvalAmount = useMemo(() => {
		...
		if (dcasCount === '' || dcasCount === '' || isNaN(parseInt(dcasCount))) return '-'
		return bnDisplay(bn(eN(amountDCA, tokenInData?.decimals)).times(dcasCount === '' ? '0' : parseInt(dcasCount)), tokenInData?.decimals, 3)
	}, [amountDCA, dcasCount, tokenInData?.decimals])

	const alreadyApproved = useMemo(() => {
		if (approvalAmount === 'Inf') return allowance === 'Inf'
		return bn(approvalAmount).lte(bnDisplay(tokenInUserData?.allowance, tokenInData?.decimals))
	}, [allowance, approvalAmount, tokenInData?.decimals, tokenInUserData?.allowance])
	...
	return (
		<>
			<IntroText small>
				Involica withdraws your spend token when you DCA. Your approval amount determines how many times you will DCA.
				<br />
				<br />
				<i>
					Enter the amount of DCA transactions you want to run, and then approve your <b>{tokenInData?.symbol}</b> spends:
					<br />
					<br />
					(Infinite Approval will continue to DCA until your wallet balance runs out or Involica manually stopped)
				</i>
			</IntroText>
			<RowStart gap='8px'>
				<NumericInput
			...
			<Column>
				<Text bold>
					Amount to approve: {alreadyApproved ? 0 : approvalAmount} {tokenInData?.symbol}
				</Text>
				{alreadyApproved && (
					<Text small italic>
			...
			<SummitButton
				disabled={alreadyApproved || dcasCount === '0' || dcasCount === '' || approvalAmount === '-'}
				isLoading={pending}

...
export const AmountIn: React.FC = () => {
	...
	const amountDcaUsd = useMemo(() => {
		if (amountDCA == null || tokenInData?.price == null || amountDCAInvalidReason != null || isNaN(parseFloat(amountDCA))) return 0
	...
	const balanceDCAs = useMemo(() => {
		if (tokenInUserData?.balance == null || tokenInData?.decimals == null) return null
		if (amountDCA == null || amountDCA === '' || isNaN(parseFloat(amountDCA)) || parseFloat(amountDCA) === 0) return null
	...
	return (
		<>
			...
			<TokenAndAmountSelector token={tokenIn} value={amountDCA} setValue={setAmountDCA} tokenSelectDisabled invalidReason={null} max={null} />
			...
			{minOut != null && amountDCAInvalidReason == null && Math.floor(amountDcaUsd * minOut.weight) / 100 < 0.1 && (
				<Text color='warning' italic mt='-12px'>
					<b>
						At this amount, your swap for {getSymbol(minOut.token)} will be ${(Math.floor(amountDcaUsd * minOut.weight) / 100).toFixed(2)}
					</b>
					<br />
